...
for b in "${graphfiles[@]}"; do
	...
	bname=${arrb[-1]}
	printf "$bname\n" 1>&2
	awk '!/^track/{print $1, $2, $3, ".", $4}' $b | SCRIPTS/bedmap --prec 3 --unmapped-val 0 --wmean mmb_temp_${temp_pref}_slopped_peaks.bed /dev/stdin > mmb_temp_${temp_pref}_map_$bname.bed
done
