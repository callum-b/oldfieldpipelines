...
printf "Bigwig files to process:\n" 1>&2
printf " <%s>\n" "${graphfiles[@]}" 1>&2
printf "\n" 1>&2
if (( ${#graphfiles[@]} == 0 )); then
    echo "No Bigwig files found, exiting" ; exit 1;
fi
...
for a in "${peakfiles[@]}"; do
	arra=(${a//\// })
	aname=${arra[${#arra[@]}-1]} ##ZSH doesn't handle -1 apparently? So len of arr -1 for last element
	printf "$aname\n" 1>&2
...
printf "Mapping merged peak file to each BigWig...\n" 1>&2

for b in "${graphfiles[@]}"; do
	arrb=(${b//\// })
	bname=${arrb[${#arrb[@]}-1]}
	printf "$bname\n" 1>&2
	SCRIPTS/bigWigToBedGraph_arm64 $b /dev/stdout | awk '!/^track/{print $1, $2, $3, ".", $4}' | SCRIPTS/bedmap_arm64 --prec 3 --unmapped-val 0 --wmean mmb_temp_${temp_pref}_slopped_peaks.bed /dev/stdin > mmb_temp_${temp_pref}_map_$bname.bed
done
